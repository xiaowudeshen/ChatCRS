...
def make_demo(item, prompt, test_prompt, instruction=None, guidance = None, test=False):
    ...
    prompt = prompt.replace("{INST}", instruction).replace("{Q}", item['Input']).replace("\u00a0", "")
    test_prompt = test_prompt.replace("{INST}", instruction).replace("{Q}", item['Input']).replace("\u00a0", " ")
...
def main():
    ...
    if args.quick_test is not None:
        ...
        logger.info("Quick test mode, total number of testing %d, only evaluate on %d samples",len(eval_data) ,len(eval_ids))
    ...
    for idx, item in enumerate(tqdm(eval_data)):
        prompt = item['prompt']
        if args.task == "REC":
            item["Output"] = [d.replace("\u00a0", " ") for d in item["Output"]]
        # print(prompt)
        ...
        for _ in range(args.num_samples):
            if args.interactive:
                ...
                # breakpoint()
                output_array.append(CRS.generate(prompt, args.max_new_tokens))
                item['prompt'] = prompt
    ...
    if args.openai_api:
        ...
        logger.info(f"The openai model used is {args.model}")
